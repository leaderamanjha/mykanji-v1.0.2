...
class ApiClient extends GetConnect {
  ...
  /// Performs API call for https://dull-jade-drill-tutu.cyclic.app/auth/category-getall/:id
  ///
  /// Sends a GET request to the server's 'https://dull-jade-drill-tutu.cyclic.app/auth/category-getall/:id' endpoint
  /// with the provided headers and request data
  /// Returns a [GetLoginAuthResp] object representing the response.
  /// Throws an error if the request fails or an exception occurs.
  Future<GetLoginAuthResp> loginAuth({
    Map<String, String> headers = const {},
    String? id = '',
  }) async {
    ProgressDialogUtils.showProgressDialog();
    try {
      await isNetworkConnected();
      Response response = await httpClient.get(
        '$url/auth/category-getall/$id',
        headers: headers,
      );
      ProgressDialogUtils.hideProgressDialog();
      if (_isSuccessCall(response)) {
        return GetLoginAuthResp.fromJson(response.body);
      } else {
        throw response.body != null
            ? GetLoginAuthResp.fromJson(response.body)
            : 'Something Went Wrong!';
      }
    } catch (error, stackTrace) {
      ProgressDialogUtils.hideProgressDialog();
